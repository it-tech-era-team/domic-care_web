      ...
      <div className="flex h-screen items-center justify-center bg-slate-50 p-4">
        <div className="max-w-md bg-white border border-red-100 rounded-3xl p-8 text-center space-y-4 shadow-xl">
          ...
          <p className="text-xs text-slate-500 leading-relaxed">
            This account is registered as a <span className="font-bold capitalize">{currentUser.role}</span>. You do not have permissions to access the Admin Panel.
          </p>
          <button
            onClick={() => router.push(currentUser.role === 'user' ? '/user/dashboard' : '/caregiver/dashboard')}
