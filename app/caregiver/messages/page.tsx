...
export default function CaregiverMessages() {
  const { currentUser, conversations, messages, sendMessage } = useCareConnect();
  
  const [activeConvId, setActiveConvId] = useState<string | null>(null);
  const [inputText, setInputText] = useState('');
  const messagesEndRef = useRef<HTMLDivElement>(null);

  // Filter conversations where this caregiver is active
  const caregiverConvs = conversations.filter(c => c.caregiverId === currentUser?.id);

  // Set first conversation active if none selected
  useEffect(() => {
    if (caregiverConvs.length > 0 && !activeConvId) {
      setActiveConvId(caregiverConvs[0].id);
    }
  }, [caregiverConvs, activeConvId]);

  // Find active conversation
  const activeConv = caregiverConvs.find(c => c.id === activeConvId);
    ...
    <div className="flex-1 bg-white rounded-3xl border border-slate-100 shadow-sm flex overflow-hidden min-h-[calc(100vh-120px)] animate-fade-in">
      ...
      <div className="w-1/3 border-r border-slate-100 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto divide-y divide-slate-50">
          {caregiverConvs.length === 0 ? (
            <div className="p-6 text-center text-slate-400 space-y-2">
              <p className="text-xs font-semibold">No active chats</p>
              <p className="text-[10px]">Chats will appear here once families message you.</p>
            </div>
          ) : (
            caregiverConvs.map((conv) => {
              ...
              return (
                ...
                >
                  <div className="h-10 w-10 rounded-xl bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-sm shrink-0 border border-blue-100">
                    {conv.userFullName.split(' ').map(n => n[0]).join('')}
                  </div>
        ...
        {activeConv ? (
          <>
            ...
            <div className="flex-1 p-4 overflow-y-auto space-y-4">
              {activeMessages.map((m) => {
                ...
                return (
                  ...
                  >
                    {!isMe && (
                      <div className="h-6 w-6 rounded-md bg-blue-50 text-blue-600 flex items-center justify-center font-bold text-[9px] border border-blue-100 shrink-0">
                        {activeConv.userFullName.split(' ').map(n => n[0]).join('')}
                      </div>
                    )}
            ...
            <form onSubmit={handleSend} className="bg-white p-3 border-t border-slate-100 flex items-center gap-3">
              ...
              <button
                type="submit"
                className="rounded-xl bg-blue-600 hover:bg-blue-700 text-white p-2.5 shadow-md shadow-blue-500/10 transition-all flex items-center justify-center cursor-pointer"
              >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-slate-400 space-y-3">
            ...
            <h3 className="font-bold text-slate-800">Select a Client Chat</h3>
            <p className="text-xs text-slate-400">Choose a family from the left list to respond to.</p>
          </div>
        )}
      </div>
